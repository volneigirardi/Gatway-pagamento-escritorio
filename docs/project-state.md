...
- **DBA gate — PASS WITH RISKS**: the `postgres-dba` final review passed. The high migration-rollback finding was fixed (migrations `009` and `010`, `010` `down` now recreates pre-010 partial unique indexes without the `deleted_at` predicate). Residual medium/operational risks are documented by the DBA: rollback of `010` is only safe before `payments`/`subscriptions` soft-deletes are written; non-concurrent unique-index rebuilds and FK-check race windows should run in a maintenance window or be refactored to `CONCURRENTLY`; integration tests require Docker.
...
The first vertical's mandatory `postgres-dba` gate is `PASS WITH RISKS` for the foundation phase. Before production: run `apps/api/test/admin-migrations.integration.spec.ts` with Docker; provision real database roles/secrets/TLS; validate against a live cluster; and resolve or formally defer the Expo/Metro `image-size` and `uuid` advisories.
